    ...
    <div className="min-h-screen bg-fl-bg flex items-center justify-center p-6">
      <div className="w-full max-w-md border border-fl-border bg-fl-surface">
        ...
        <div className="px-8 py-8 space-y-6">
          ...
          {error.digest && (
            <p className="font-mono text-fl-hint text-fl-muted-2 tracking-wide">
              {t('digest')}: <span className="text-fl-muted-1">{error.digest}</span>
            </p>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button
              onClick={reset}
              className="flex-1 font-mono text-xs tracking-widest uppercase px-6 py-3 bg-fl-fg text-fl-bg hover:bg-fl-fg-bright transition-colors"
            >
              {t('retry')}
            </button>
            <a
