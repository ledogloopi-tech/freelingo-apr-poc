...
| `build_lesson_generation_prompt()`           | `prompts/lesson.py`        | `services/lesson_generator.py`      | `system` via `structured_output` | `LessonContent` JSON, including optional lesson-level `native_explanation` with translated explanation, common traps, and mini-glossary, optional per-exercise `native_explanation` strings, and enriched vocabulary items with optional native-language translation/example support. |
...
| `build_native_exercise_explanation_on_demand_prompt()` | `prompts/lesson.py`        | `routers/lessons.py`                | `user` via `structured_output`   | `NativeExerciseExplanationResponse` JSON for generating one missing exercise-level native explanation on demand. |
...
| Native exercise explanation  | `NATIVE_EXERCISE_EXPLANATION_ON_DEMAND`                                  | Generates one concise native-language clarification for an existing exercise from its type, question, correct answer, and target-language explanation, then caches it in `lesson.content.exercises[*].native_explanation`. |
...
- Lesson generation: `lesson_type`, `topic`, `unit_id`, `grammar_points`, `vocabulary_set_ids`, `week`, `day`, `valid_slugs`, optional `native_language_name` for lesson-level and per-exercise `native_explanation` and native-language vocabulary support.
