...
| `build_regenerate_exercise_prompt()`         | `prompts/lesson.py`        | `services/lesson_generator.py`      | `system` via `structured_output` | `ExerciseContent` JSON for replacing one invalid unanswered lesson exercise with the same exercise type. |
...
| `build_native_exercise_hint_on_demand_prompt()` | `prompts/lesson.py`        | `routers/lessons.py`                | `user` via `structured_output`   | `NativeExerciseHintResponse` JSON for generating one missing pre-answer native hint on demand without revealing the answer. |
