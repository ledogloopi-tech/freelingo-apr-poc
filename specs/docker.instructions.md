...
| `kokoro` | `ghcr.io/remsky/kokoro-fastapi-gpu:latest` | 8880 (internal) | 2 | TTS — official Kokoro-FastAPI image (CUDA 12.6, sm_75+). Only needed when `TTS_PROVIDER=local`. Blackwell (RTX 50) users should use `:latest-cu128`. |
...
- Official upstream image (CUDA 12.6); supports Turing+ GPUs (sm_75+)
- Blackwell / RTX 50-series: use `:latest-cu128` tag for cu128 optimisations
...
|---------|-----------|-----------|---------------|
| Kokoro TTS | `ghcr.io/remsky/kokoro-fastapi-gpu:latest` | `ghcr.io/remsky/kokoro-fastapi-cpu:latest` | Replace image; remove `deploy` block |
